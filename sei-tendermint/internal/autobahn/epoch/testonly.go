...
// so the window covers [startEpoch-1, startEpoch, startEpoch+1].
...
func GenRegistry(rng utils.Rng, size int) (*Registry, []types.SecretKey, types.EpochIndex) {
	...
	// ([0,1] or [0,1,2]), so tests that build CommitQC chains from road index 0
	// can still look up epoch 0 in the window. Higher values would require all
	// such tests to anchor their chains at startEpoch*EpochLength.
...
// Seeds [startEpoch-1, startEpoch, startEpoch+1] so TrioAt(startEpoch*EpochLength) works.
// Intended for use in tests only.
